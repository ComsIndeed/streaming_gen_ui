...
class WidgetRegistry {
  ...
  static String _generateDefaultJsonExample(String namespace, Map<String, String> properties) {
    ...
    for (final entry in properties.entries) {
      ...
      if (typeDesc.contains('list')) {
        ...
        mockValue = 'true';
      } else if (typeDesc.contains('int') || typeDesc.contains('num') || typeDesc.contains('double') || typeDesc.contains('float')) {
        if (key.contains('rating') || key.contains('score')) {
          mockValue = '4.8';
        } else if (key.contains('price') || key.contains('cost') || key.contains('amount')) {
          mockValue = '99.99';
        } else if (key.contains('progress') || key.contains('ratio') || key.contains('percent')) {
          mockValue = '0.75';
        } else if (key.contains('width') || key.contains('height') || key.contains('size') || key.contains('radius')) {
          mockValue = '150';
        } else {
          mockValue = '42';
        }
      } else {
        if (key.contains('url') || key.contains('image') || key.contains('photo') || key.contains('avatar')) {
          mockValue = '"https://example.com/image.png"';
        } else if (key.contains('email')) {
          mockValue = '"user@example.com"';
        } else if (key.contains('title') || key.contains('header') || key.contains('subject')) {
        ...
        } else if (key.contains('description') || key.contains('bio') || key.contains('content') || key.contains('text')) {
          mockValue = '"This is a beautiful, interactive card component built with streaming_gen_ui."';
        } else if (key.contains('name') || key.contains('author') || key.contains('user')) {
          mockValue = '"John Doe"';
        } else if (key.contains('status') || key.contains('badge') || key.contains('state')) {
          mockValue = '"active"';
        } else if (key.contains('color') || key.contains('hex')) {
          mockValue = '"#ff0055"';
        } else if (key.contains('date') || key.contains('time')) {
          mockValue = '"Just Now"';
        } else if (key.contains('action') || key.contains('callback') || key.contains('event')) {
