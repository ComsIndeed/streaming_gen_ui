# streaming_gen_ui (Documentation)
...
![Demo: Hero visual showing a complete end-to-end real-time chat session. The LLM streams normal Markdown text, then starts an <interface> block that smoothly scales/fades in a profile configuration widget. The widget fields populate character-by-character, and the Save button morphs from a disabled grey state into a fully active blue pill when the stream resolves.](https://raw.githubusercontent.com/ComsIndeed/streaming-gen-ui/main/demos/hero_streaming_chat.gif)
...
![Demo: Visualizing the three morphing states side-by-side or sequentially. The placeholder starts as a grey shimmer container (Stage 0), morphs smoothly into an expanding form card filled with typewriter text (Stage 1), and then locks its layout bounds and activates interactive features once complete (Stage 2).](https://raw.githubusercontent.com/ComsIndeed/streaming-gen-ui/main/demos/continuous_state_morphing.gif)
...
* **The Ghostly Entry:** Elements fade and scale in softly from a local origin (`scale: 0.96` to `1.0`, `opacity: 0.0` to `1.0` over `200ms` with snap curves) rather than popping onto the screen.
  ...
  ![Demo: Soft entrance animation showing a dynamic component scaling smoothly from 0.96 to 1.0 and fading from opacity 0.0 to 1.0 using Curves.easeOutCubic over a 200ms duration. The background remains locked during this fade-in.](https://raw.githubusercontent.com/ComsIndeed/streaming-gen-ui/main/demos/ghostly_entry.gif)
...
* **Procedural Layout Growth:** Containers automatically interpolate height and border boundaries using an implicit size-tracking `AnimatedSize` wrapper.
  ...
  ![Demo: A layout card container expanding its height and boundaries dynamically via AnimatedSize. Sibling text cards beneath it are smoothly pushed downward rather than snapping.](https://raw.githubusercontent.com/ComsIndeed/streaming-gen-ui/main/demos/procedural_layout_growth.gif)
...
* **Disabled-to-Active Transmutations:** Interactive elements (buttons, inputs) remain visually muted and disabled during streaming. The exact millisecond the action property resolves, the button triggers a micro-scale bounce and morphs into its primary theme color.
  ...
  ![Demo: A button starting in a neutral grey, slightly shimming disabled state while its text streams. Upon stream completion, the button triggers a micro-bounce (scale down/up) and morphs its background color dynamically into a rich theme color, signaling that it is now active.](https://raw.githubusercontent.com/ComsIndeed/streaming-gen-ui/main/demos/disabled_to_active_transmutation.gif)
...
* **`StreamingImage`:** Pulse-shimmers a matching aspect-ratio skeleton before cross-fading into the network image once the URL stream completes.
  ...
  ![Demo: A custom image component displaying a pulsing skeleton container matching its aspect ratio. When the image URL stream completes and loads, it smoothly cross-fades into the network image using an AnimatedSwitcher.](https://raw.githubusercontent.com/ComsIndeed/streaming-gen-ui/main/demos/streaming_image_card.gif)
